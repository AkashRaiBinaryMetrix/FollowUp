...
<div class="mls-searchbar">
 <div class="container">
  
  <div class="mls-filter-phn">Filters <span><i class="las la-plus"></i></span></div> 
   
  <form class="mls-form" name="cform" action="{{ route('search.queryfilters') }}" method="post">
    @csrf
    <div class="row">
      
      <div class="col">
       <div class="form-group">
     <input type="text" value="{{ $find_location ?? '' }}" class="form-control" name="find-location" id="find-location" placeholder="City, address, Zip or MLS#" required="">
     <span class="field-icon"><i class="las la-map-marker"></i></span>
    </div>
      </div>
      
    <div class="col">
       <div class="form-group">
      <select id="frentsale" name="frentsale" class="form-control">
        <option value="rent" {{ ($frentsale == "rent" ? "selected":"") }}>For Rent</option>
        <option value="sale" {{ ($frentsale == "sale" ? "selected":"") }}>For Sale</option>
      </select>
     <span class="field-icon"><i class="las la-money-bill"></i></span>
    </div>
      </div>
            
      <div class="col">
       <div class="form-group">
      <select id="minmax-price" name="minmax-price"  class="form-control">
        <option>Min-Max Price</option>
        <option value="$1000 - $2000" {{ ($minmaxprice == "$1000 - $2000" ? "selected":"") }}>$1000 - $2000</option>
        <option value="$3000 - $5000" {{ ($minmaxprice == "$3000 - $5000" ? "selected":"") }}>$3000 - $5000</option>
        <option value="$6000 - $8000" {{ ($minmaxprice == "$6000 - $8000" ? "selected":"") }}>$6000 - $8000</option>
        <option value="$9000 - $10000" {{ ($minmaxprice == "$9000 - $10000" ? "selected":"") }}>$9000 - $10000</option>
        <option value="$12000 - $14000" {{ ($minmaxprice == "$12000 - $14000" ? "selected":"") }}>$12000 - $14000</option>
      </select>
     <span class="field-icon"><i class="las la-money-bill"></i></span>
    </div>
      </div>
    ...
    <div class="col">
      ...
      <select id="beds" name="beds" class="form-control">
        ...
        <option value="5" {{ ($beds == "4" ? "selected":"") }}>5</option>

      </select>
     <span class="field-icon"><i class="las la-bed"></i></span>
    </div>
      </div>
    ...
    <div class="col">
      ...
      <select id="baths" name="baths" class="form-control">
        ...
        <option value="5" {{ ($baths == "4" ? "selected":"") }}>5</option>
      </select>
     <span class="field-icon"><i class="las la-shower"></i></span>
    </div>
      </div>  
            
    <div class="col">
       <div class="form-group mls-submit-btn">
      <button type="submit" class="common-btn" name="filterBtnSubmit">Search <i class="las la-search"></i> </button>
    </div>
      </div>    
     ...
     <div class="mls-listing-map">
       ...
       @php
         foreach($result as $booking){
            $address1 = str_replace(" ", "+", $booking->address);
    ...
    <script type="text/javascript"
     ...
     <div class="col-lg-6 p-0">
      <div class="mls-listing-sec">
       ...
       <h3> Real Estate Listings <small>6 Search results</small></h3>
       <div class="listing-sorting">
         <select id="sorting" class="form-control">
          <option>Price ( Low to High )</option>
          <option>Price ( High to Low )</option>
          <option>Featured Listing</option>
          <option>Popular Listing</option> 
        </select>  
       </div>
       </div>
       ...
       <div class="col-lg-6 col-md-4 col-sm-6">
         <div class="pro-list-col">
          ...
          <div class="pro-list-text">
           ...
           <p class="proper-name"><i class="las la-map-marker"></i>{{$booking->address}},{{$booking->city}},{{$booking->state}}-{{$booking->zipcode}}</p>
             <div class="pro-area">

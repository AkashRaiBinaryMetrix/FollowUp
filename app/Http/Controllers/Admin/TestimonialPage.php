...
class TestimonialPage extends Controller
{
    ...
    public function addUpdateTestimonialPage(Request $request) {
           $post = $request->input();
          ...
          if(!empty($id)) {
                $update = DB::table('testimonial')->where('id',$id)->update($aData);
                if($update) {
                     $request->session()->flash('status', 'success');
                    $request->session()->flash('successMsg','Testimonial created successfully');
                } else {
                    $request->session()->flash('failureMsg','Something went wrong');
                }
          } else {
               $insert = DB::table('testimonial')->insertGetId($aData);
               if($insert) {
                    $request->session()->flash('status', 'success');
                    $request->session()->flash('successMsg','Testimonial created successfully');
               } else {
    ...
    function deleteTestimonial(Request $request){
        DB::table('testimonial')->delete($request->id);
    }

    function testimonialedit(Request $request,$id){
        /*---------------------- get per page paging record show ------------------------------*/
        $iPerPagePagination  = perPagePaging();
        /*---------------------- get per page paging record show ------------------------------*/
        ...
        $aDetail = DB::table('testimonial')->where('id',$id)->first();

        return view('admin.testimonial.updateTestimonial', ['aLists' => $aLists, 'aDetail' => $aDetail]);
    }
